...
const options = {	//setting pfx
    pfx: fs.readFileSync(__dirname+'/ssl/'),  // FileAddress
    passphrase: '' //Password
};
...
var requestAddress = "https://localhost";    // back api
...
app.post("/Register", function(req,res){
    ...
    if(email != undefined && password != undefined){
        axios.get(requestAddress + "/Home/SendRegisterEmail" + requestKey + "&Email=" + email + "&Password=" + password).then(function (response) {
...
app.get("/Register/Validate",function (req,res,next) {
    if(req.url != "/Validate") {
        axios.get(requestAddress + "Home/RegisterValidate" + requestKey + "&ValidateKey=" + req.url.substring(1,16)).then(function (response) {
            if(response.data.Result == true) {
                res.send("注册成功");
            }else {
                next();
